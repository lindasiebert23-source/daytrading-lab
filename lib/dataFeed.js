...
// Fetches up to `limit` candles, paginating backwards in time in batches of
// 1000 (Binance's per-request max) if more history is requested than a
// single call can return.
async function fetchKlines(coinId = "bitcoin", interval = "1h", limit = 500) {
  ...
  for (const base of BINANCE_HOSTS) {
    try {
      let all = await fetchKlinesPage(base, symbol, interval, Math.min(limit, 1000));
