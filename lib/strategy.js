...
const RANDOM_ENTRY_PARAMS = {
  ...PARAMS,
  entryProbPerBar: 0.02,
  seed: 42,
};
...
function generateRandomEntrySignals(candles, params = RANDOM_ENTRY_PARAMS, costs = DEFAULT_COSTS) {
  const closes = candles.map(c => c.close);
  const highs = candles.map(c => c.high);
  const lows = candles.map(c => c.low);
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    if (roll < params.entryProbPerBar) {
      diagnostics.triggerCandidates++;
      const side = uptrend ? "LONG" : "SHORT";
      const sign = side === "LONG" ? 1 : -1;
      const entryPrice = closes[i];
      const rawStopDistance = atrValues[i] * params.atrStopMultiple;
      const rawStopDistancePct = (rawStopDistance / entryPrice) * 100;
      ...
      if (validation.valid) {
        ...
        signals[i] = {
          side,
          reason: side === "LONG" ? "RANDOM_ENTRY" : "RANDOM_ENTRY_SHORT",
          entryPrice,
          stopDistance: rawStopDistance,
          stopDistancePct: rawStopDistancePct,
          stopPrice: entryPrice - sign * rawStopDistance,
...
module.exports = {
  ...
  RANDOM_ENTRY_PARAMS, generateRandomEntrySignals,
};
